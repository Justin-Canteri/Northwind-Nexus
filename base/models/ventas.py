...
from django.db import models
from .inventory import Product  # Importamos Product para la relación en OrderDetail
...
class Order(models.Model):
    ...
    # Nota: Aquí asumo que Employee está en rrhh.py, si no, usa 'rrhh.Employee'
    ...
    freight = models.DecimalField(db_column='freight', max_length=100, max_digits=10, decimal_places=2, blank=True, null=True)
    ...
    def __str__(self):
        return f"Orden {self.id} - {self.customer.company_name if self.customer else 'Sin Cliente'}"

class OrderDetail(models.Model):
    # En Northwind, esta tabla suele tener una clave primaria compuesta (OrderID + ProductID)
    # Como Django no soporta bien claves compuestas nativas, usamos la referencia a Order
    ...
    class Meta:
        ...
        # Esto es importante para que Django reconozca la relación doble como clave única
